...
r_squared_chart
```
We first compared all the R^2 for each team statistic variable. By doing this we were able to rule out the majority of the team statistic variables that didn't influence the win-loss percentage. When looking at the R^2 values we saw that block assists and total blocks have the two highest values. We then took these two variables and started looking at their simple linear regression plots and four diagnostic plots.
...
```{r triple_block_plot, echo = FALSE, message = FALSE, warning = FALSE}
